...
% Last Modified by GUIDE v2.5 14-Mar-2017 16:32:12
...
msg{2} = sprintf('Tombol Preprocessing = Deteksi Tepi Canny');
msg{3} = sprintf('Tombol Save = Untuk menyimpan file');
msg{4} = sprintf('Tombol Reset = Untuk mereset tampilan / mengosongkan tampilan');
msg{5} = sprintf('Tombol Hitung = Untuk proses menghitung diameter');
msg{6} = sprintf('Tombol Exit = Keluar dari aplikasi');
msg{7} = sprintf('Tombol Help = Bantuan untuk menggunakan aplikasi ini');
msg{8} = sprintf('Tombol About = Informasi dari aplikasi ');
